...
class FriendshipUpdate {
  ...
  static Future<void> addProgress(String userID1, String userID2, DocumentSnapshot friendshipDoc, {required double exp}) async {
    ...
    if (progress >= 1) {
      ...
      await Constants.usersCollection.doc(userID1).update({
        Constants.powerDoc: FieldValue.increment(1)
      });
      await Constants.usersCollection.doc(userID2).update({
        Constants.powerDoc: FieldValue.increment(1)
      });
    } else {
      await Constants.friendshipsCollection.doc(friendshipDoc.id).update({
